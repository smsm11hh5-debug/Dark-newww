...
def send_welcome(message):
  ...
  markup.add(
      types.KeyboardButton("📊 حالة السبام"), types.KeyboardButton("🏓 بينغ")
  )
  help_text = (
      "أهلاً بك في مقبره skip 👾\n\n"
      "الأوامر المتاحة:\n"
      "☺️ /spam <UID> - لبدء إرسال طلبات الصداقة\n"
      "🥱 /stop <UID> - لإيقاف السبام عن آيدي معين\n"
      "🖥️ /status - لمعرفة الآيديات النشطة حالياً\n"
      "🛡️ /ping - لفحص سرعة استجابة السيرفر\n"
...
def cmd_ping(message):
  bot.reply_to(message, "يعمل بكامل قوه وسرعه 🖥️")
...
def cmd_status(message):
  if not aCtive:
    bot.reply_to(
        message, "👍🏻 لا توجد أي عمليات سبام نشطة حالياً.", parse_mode="Markdown"
    )
    return

  text = "😔 **الآيديات التي يتم سبامها حالياً:**\n"
...
def cmd_spam(message):
  ...
  if len(parts) < 2:
    msg = bot.reply_to(
        message, "أرسل لي **الايدي (UID)** المستهدف الآن للبدء:"
    )
...
def btn_start_spam(message):
  msg = bot.reply_to(
      message, "أرسل لي **الايدي (UID)** المستهدف لبدء السبام:"
  )
  bot.register_next_step_handler(msg, process_inline_spam)
...
def start_spam_process(message, uid):
  if not uid.isdigit():
    bot.reply_to(message, "❌ الآيدي يجب أن يكون أرقاماً صحيحة فقط!")
    return

  if uid in aCtive:
    bot.reply_to(
        message,
        f"⚠️ الآيدي `{uid}` يتم سبامه بالفعل مسبقاً!",
        parse_mode="Markdown",
    )
    return
  ...
  if not aCcs:
    bot.reply_to(
        message,
        "❌ ملف الحسابات `accs.txt` فارغ أو غير موجود!",
        parse_mode="Markdown",
    )
    return
...
def cmd_stop(message):
  ...
  if len(parts) < 2:
    msg = bot.reply_to(
        message, "أرسل لي **الايدي (UID)** المراد إيقاف السبام عنه:"
    )
...
def stop_spam_process(message, uid):
  ...
  if uid in aCtive:
    ...
    bot.reply_to(
        message,
        f"⚠️ الآيدي `{uid}` ليس قيد السبام أصلاً.",
        parse_mode="Markdown",
    )
...
@bot.message_handler(func=lambda message: message.text == "🏓 بينغ")
...
def sTart():
  ...
  if uid in aCtive:
    return jsonify(
        {"status": True, "msg": "Already spamming", "DEV": "JAGWAR KING"}
    )
  ev = threading.Event()
  aCtive[uid] = ev
  threading.Thread(target=sPam, args=(uid,), daemon=True).start()
  return jsonify({"status": True, "msg": "Success", "DEV": "JAGWAR KING"})
...
def sTop():
  ...
  if uid in aCtive:
    aCtive[uid].set()
    return jsonify({"status": True, "msg": "Stopping", "DEV": "JAGWAR KING"})
  return jsonify({"status": False, "msg": "Not spamming", "DEV": "JAGWAR KING"})
...
if __name__ == "__main__":
  if not TELEGRAM_TOKEN:
    print("❌ خطأ: التوكن غير موجود. تأكد من ضبط config.json قبل التشغيل.")
  else:
