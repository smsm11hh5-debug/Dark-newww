...
# استعادة الأهداف القديمة عند تشغيل السكربت تلقائياً
def restore_targets():
  ...
  if saved_list:
    print(
        f"🔄 تم استعادة وإعادة تشغيل هجمات السبام لـ {len(saved_list)} أهداف بنجاح!"
    )


# ==================== واجهة أوامر التليجرام ====================
...
def send_welcome(message):
  ...
  help_text = (
      "💀 **لوحة تحكم هجمات فري فاير (SKIP PANEL)** 🔥\n\n"
      "⚡ **الأوامر المتاحة:**\n"
      "🎯 `/spam <UID>` - فحص الهدف وبدء الهجوم الشامل\n"
      "🛑 `/stop <UID>` - إيقاف الهجوم عن الآيدي المحدد\n"
      "📋 `/status` - عرض الأهداف النشطة حالياً في السيرفر\n"
      "📂 `/accounts` - إحصائيات حسابات السبام والاتصال\n"
      "📶 `/ping` - قياس سرعة استجابة السيرفر الحقيقية (ms)\n"
      "👑 `/dev` - معلومات المطور وصانع السكربت\n"
      "🔄 `/restart` - تصفير طابور العمليات وإعادة الضبط\n\n"
      "✨ *ملاحظة: السكربت يحفظ الأهداف تلقائياً ويستعيدها عند إعادة التشغيل.*"
  )
  bot.reply_to(
...
def cmd_ping(message):
  ...
  msg = bot.reply_to(
      message, "📡 **جاري قياس استجابة السيرفر وتليجرام...**", parse_mode="Markdown"
  )
  end_time = time.time()
...
def cmd_dev(message):
  text = (
      "👑 **معلومات المطور:**\n\n"
      "👤 **المطور الأصلي:** skip\n"
      "🔥 **الإصدار:** Ultra Pro v3.1 (Auto-Restore)\n"
      "🛡️ **حالة السيرفر:** مؤمن ومستقر تماماً ومخصص لهجمات السبام السريع."
  )
...
def cmd_restart_bot(message):
  ...
  bot.reply_to(
      message,
      "🔄 **تم عمل إعادة ضبط طارئة للسيرفر!**\n🧹 تم مسح كافة العمليات العالقة وملف الأهداف الحفظ وتصفير الذاكرة بنجاح.",
      parse_mode="Markdown",
  )
...
def cmd_accounts(message):
  ...
  text = (
      "📊 **إحصائيات حسابات الهجوم:**\n\n"
      f"📂 **إجمالي الحسابات في الملف:** `{total}`\n"
      f"🟢 **الحسابات الجاهزة (متصلة):** `{active}`\n"
      f"🔴 **الحسابات التي تحتاج تحديث:** `{dead}`\n\n"
      "⚡ **النظام يعمل بأقصى طاقة توزيعية.**"
  )
  bot.reply_to(message, text, parse_mode="Markdown")
...
def cmd_status(message):
  if not aCtive:
    bot.reply_to(
        message,
        "💤 **لا توجد أي أهداف مستهدفة بالسبام في هذه اللحظة.**",
        parse_mode="Markdown",
    )
    return

  text = "🎯 **قائمة الضحايا (الأيديات تحت الهجوم):**\n\n"
...
def start_spam_process(message, uid):
  ...
  if uid in aCtive:
    bot.reply_to(
        message,
        f"⚠️ **تنبيه:** الآيدي `{uid}` يتم استهدافه بالهجوم بالفعل حالياً!",
        parse_mode="Markdown",
    )
    return
  ...
  wait_msg = bot.reply_to(
      message,
      f"🔍 **جاري فحص وجلب بيانات اللاعب `{uid}` من سيرفرات فري فاير...**",
      parse_mode="Markdown",
  )
  ...
  success_text = (
      f"🚀 **تم تأكيد الهدف وبدء هجوم السبام بنجاح!**\n\n"
      f"👤 **اسم اللاعب:** `{player_name}`\n"
      f"🎯 **الآيدي المستهدف:** `{uid}`\n"
      f"🔥 **الحالة:** تم إطلاق جميع الحسابات نحو الهدف بنجاح تام وتم الحفظ!"
  )
...
def stop_spam_process(message, uid):
  ...
  if uid in aCtive:
    ...
    bot.reply_to(
        message,
        f"⚠️ **تنبيه:** الآيدي `{uid}` ليس عليه أي عمليات هجوم نشطة حالياً.",
        parse_mode="Markdown",
    )
...
def sTart():
  ...
  threading.Thread(target=sPam, args=(uid,), daemon=True).start()
  save_targets_to_file()
  return jsonify({"status": True, "msg": "Attack Started Successfully", "DEV": "skip"})
...
def run_telegram_bot():
  print("🚀 Telegram Bot is running live with Skip modifications...")
...
if __name__ == "__main__":
  if not TELEGRAM_TOKEN:
    ...
    # استعادة الهجمات السابقة قبل تشغيل السيرفر والبوت
    restore_targets()
